...
//Obtener Marcas de Cerveza
const getMarcasCerveza = (req, res) =>{
    db.query(
        `SELECT DISTINCT m.nombreMarca
        FROM Marca m
        JOIN Producto p ON m.id_marca = p.id_marca
        JOIN Categoria c ON p.id_categoria = c.id_categoria
        WHERE c.nombreCategoria ='Cerveza'`,
        (err, results)=>{
            if(err){
...
// Contar marcas de cerveza 
const getCountMarcasCerveza = (req, res) => {
    db.query(`
...
// Se encarga de obtener todos los productos de la base de datos  solo los que pertenecen cerveza
const getAllProductCerveza = (req,res)=>{
    const query= `SELECT * 
        FROM Producto p
        JOIN Categoria c ON p.id_categoria = c.id_categoria
        WHERE c.nombreCategoria = 'Cerveza';`; //Consulta para ver todos lo produtos 
...
// Obtener las presentaciones únicas de la categoria cerveza
const getPresentacionesCerveza = (req, res) => {
    db.query(  `
        SELECT DISTINCT p.presentacion_ml 
        FROM Producto p
        JOIN Categoria c ON p.id_categoria = c.id_categoria
        WHERE c.nombreCategoria = 'Cerveza' AND p.presentacion_ml IS NOT NULL
  ...
  // Obtener las presentaciones únicas de la categoria cerveza
  const getPresentacionesCervezaCount = (req, res) => {
    db.query(
        `
 ...
 module.exports = { getMarcasCerveza,
