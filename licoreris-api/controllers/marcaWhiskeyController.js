...
// Obtener MarcasWhiskey
const getMarcasWhiskey= (req, res) => {
    db.query(
        `SELECT DISTINCT m.nombreMarca
         FROM Marca m
         JOIN Producto p ON m.id_marca = p.id_marca
         JOIN Categoria c ON p.id_categoria = c.id_categoria
         WHERE c.nombreCategoria = 'Whiskey'`,
        (err, results) => {
            if (err) {
...
//  Contar marcas de Whiskey 
const getCountMarcasWhiskey = (req, res) => {
    db.query(`
...
// Se encarga de obtener todos los productos de la base de datos  solo los que pertenecen Whiskey
const getAllProductWhiskey = (req,res)=>{
    const query= `SELECT * 
        FROM Producto p
        JOIN Categoria c ON p.id_categoria = c.id_categoria
        WHERE c.nombreCategoria = 'Whiskey';`; //Consulta para ver todos lo produtos 
...
// Obtener las presentaciones únicas de la categoria Whiskey
const getPresentacionesWhiskey = (req, res) => {
    db.query(  `
        SELECT DISTINCT p.presentacion_ml 
        FROM Producto p
        JOIN Categoria c ON p.id_categoria = c.id_categoria
        WHERE c.nombreCategoria = 'Whiskey' AND p.presentacion_ml IS NOT NULL
  ...
  const getPresentacionesWhiskeyCount = (req, res) => {
    db.query(
        `
 ...
 module.exports = { getMarcasWhiskey, getCountMarcasWhiskey,getAllProductWhiskey, getPresentacionesWhiskey, getPresentacionesWhiskeyCount};
